...
export const commandRegistry: CommandSpec[] = [
  ...
  },
  {
    ...
    "description": "For work.annotate, instruction must be a nonempty JSON object containing only project_ref, priority, next_actor, next_action, blockers, wake_condition, start_at, due_at, relations, executions. Keys and value shapes are validated before proposal insertion; invalid keys or shapes return 400 invalid_request naming the offending keys and the allowed set. Requires the current active lease token in the request body. Missing tokens return 409 lease_required; expired, released, or stale tokens return 409 lease_mismatch and require claiming again.",
...
export class AnxClient {
  ...
  docsPut(pathParams: Record<string, string>, options: RequestOptions = {}): Promise<InvokeResult> {
  ...
  docsSearch(options: RequestOptions = {}): Promise<InvokeResult> {
