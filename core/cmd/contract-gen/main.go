...
func writeGoClient(goOutDir string, commands []command, modulePath string) error {
	...
	src.WriteString("\tMaxResponseBytes int64\n")
	src.WriteString("}\n\n")

	src.WriteString("var ErrResponseTooLarge = errors.New(\"workspace response exceeds max_response_bytes\")\n\n")
	...
	src.WriteString("\t}\n")
	src.WriteString("\treader := resp.Body\n")
	src.WriteString("\tif opts.MaxResponseBytes > 0 {\n")
	src.WriteString("\t\treader = io.NopCloser(io.LimitReader(resp.Body, opts.MaxResponseBytes+1))\n")
	src.WriteString("\t}\n")
	src.WriteString("\tbodyBytes, readErr := io.ReadAll(reader)\n")
	...
	src.WriteString("\t}\n")
	src.WriteString("\tif opts.MaxResponseBytes > 0 && int64(len(bodyBytes)) > opts.MaxResponseBytes {\n")
	src.WriteString("\t\treturn resp, nil, ErrResponseTooLarge\n")
	src.WriteString("\t}\n")
