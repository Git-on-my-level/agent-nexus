...
- Open PM turns require an active lease for complete, fail, proposal and context operations. An unclaimed/released/lease-expired turn returns `409 conflict` with "this turn is not claimed; claim it first", even with a stale token. Complete/fail also require the matching token. Identical terminal completion/failure replays also refuse a cleared lease; they make no mutation. Past turn deadlines still return `turn_closed`.
