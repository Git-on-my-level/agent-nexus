...
- Human principals proposing through a channel retain `origin_kind: human`; the existing `origin` field stores the validated channel identity. A pending human proposal blocks a non-human proposal only for the same work and scope, so `work.annotate` and `work.phase` do not block each other.
