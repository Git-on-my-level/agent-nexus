...
- `work.annotate` proposal `instruction` must be a nonempty JSON object. Allowed keys are `project_ref`, `priority`, `next_actor`, `next_action`, `blockers`, `wake_condition`, `start_at`, `due_at`, `relations`, and `executions`. Both proposal endpoints validate keys and canonical local value shapes before insertion; invalid input returns `400 invalid_request` naming offending keys and the allowed set.
