...
const events = [
  ...
  // ── Human inbox queue (explicit human_attention_requested drives /inbox) ──
  {
    ...
    payload: {
      ...
      response_proposals: [
        "Confirmed — POS and inventory match production; proceed with Riverside handoff using the current checklist.",
        "Request updates first: add one more production POS screenshot to the checklist section before sign-off.",
        "Pause handoff until the inventory subsection names a backup garnish supplier explicitly.",
        "Approve content only; defer SqueezeBot 2000 cutover scheduling to Ops (separate inbox item).",
      ],
    },
  ...
  },
  {
    ...
    payload: {
      ...
      requester_label: "SupplyRover",
      response_proposals: [
        "Approve — place the full 100-unit LocalGrove PO now while CitrusBot is offline; CitrusFresh stays primary when API returns.",
        "Reject — wait for CitrusFresh's counter-offer even if CitrusBot outage extends; prioritize contract leverage.",
        "Approve capped at 80 units tonight; reassess CitrusBot recovery at 06:00 before releasing the remainder.",
        "Defer 6 hours — have Jordan sanity-check CitrusFresh SLA credits before approving any emergency buy.",
      ],
    },
    provenance: { sources: ["seed:human-inbox"] },
  },
  {
    ...
    payload: {
      ...
      requester_label: "Till-E",
      coverage_hint:
        "Tradeoff space: staffing vs peel waste; aligns with Till-E throughput model and Riverside morning spike forecast.",
      response_proposals: [
        "Use 06:30–07:00 double batch as proposed; matches forecast rush and minimizes idle peel time afterward.",
        "Shift earlier to 06:00–06:45 double batch only; acceptable if lighter traffic outweighs warmup labor cost.",
        "Single batch until labor texts open — avoid double-batch waste if the opener shift is short-handed.",
        "Split strategy: double batch lemonade only from 06:45; keep tea single-batch due to slower prep downstream.",
        "Pilot double batch weekdays only — keep weekend single-batch until spoilage telemetry drops below 3%.",
        "Pause double-batch rollout — rerun forecast after tomorrow’s civic event crowdsourcing signal posts.",
      ],
    },
    provenance: { sources: ["seed:human-inbox"] },
  },
  {
    ...
    payload: {
      ...
      requester_actor_id: "actor-ops-ai",
      requester_label: "OpsAI",
      severity: "high",
      response_proposals: [
        "Label dried botanical garnish generically (‘contains trace botanical allergens’ advisory) — ship lavender on menu with guardrail training.",
        "Remove lavender garnish from the printed menu until Legal posts approved phrasing — keep lavender off POS until then.",
        "Pause the entire summer garnish line until Regulatory shares an explicit citation; run menu without botanical garnishes temporarily.",
        "Ship with lavender listed as garnish only plus staff script to verbally confirm allergies when orders mention botanicals.",
      ],
    },
    provenance: { sources: ["seed:human-inbox"] },
  },
