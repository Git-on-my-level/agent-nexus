...
{#if hasDetails}
  <details class="group inline-block">
    <summary
      class="inline-flex cursor-pointer list-none items-center gap-1.5 text-micro text-fg-muted select-none hover:text-fg"
    >
      <span class={`h-1.5 w-1.5 rounded-full ${dotClass}`}></span>
      {label}
    </summary>
    <div
      class="mt-1 rounded border border-line bg-bg-soft px-3 py-2 text-micro text-fg-muted"
    >
      {#if sources.length > 0}
      ...
      {#if provenance?.by_field}
        <details class="mt-1">
          ...
          >
          <pre
            class="mt-1 overflow-auto rounded bg-bg-soft p-2 text-micro">{JSON.stringify(
              provenance.by_field,
              null,
              2,
            )}</pre>
...
{:else}
  <span class="inline-flex items-center gap-1.5 text-micro text-fg-muted">
    <span class={`h-1.5 w-1.5 rounded-full ${dotClass}`}></span>
    {label}
  </span>
{/if}
