...
{#if hasDetails}
  <details class="group inline-block">
    <summary
      class="inline-flex cursor-pointer list-none items-center gap-1.5 text-[11px] text-[var(--fg-muted)] select-none hover:text-[var(--fg)]"
    >
      <span class={`h-1.5 w-1.5 rounded-full ${dotClass}`}></span>
      {label}
    </summary>
    <div
      class="mt-1 rounded border border-[var(--line)] bg-[var(--bg-soft)] px-3 py-2 text-[11px] text-[var(--fg-muted)]"
    >
      {#if sources.length > 0}
      ...
      {#if provenance?.by_field}
        <details class="mt-1">
          <summary
            class="cursor-pointer text-[11px] text-[var(--fg-muted)]"
            >Field details</summary
          >
          <pre
            class="mt-1 overflow-auto rounded bg-[var(--bg-soft)] p-2 text-[11px]">{JSON.stringify(
              provenance.by_field,
              null,
              2,
            )}</pre>
...
{:else}
  <span
    class="inline-flex items-center gap-1.5 text-[11px] text-[var(--fg-muted)]"
  >
    <span class={`h-1.5 w-1.5 rounded-full ${dotClass}`}></span>
    {label}
  </span>
