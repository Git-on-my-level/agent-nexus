...
<details class="rounded-md border border-line bg-bg-soft {className}">
  <summary
    class="cursor-pointer px-4 py-2.5 text-micro text-fg-muted hover:text-fg"
    >IDs & integrity</summary
  >
  <div class="space-y-3 border-t border-line-subtle px-4 pb-3 pt-3">
    {#each rows as row}
      {#if isPresent(row.value)}
        <div>
          <p class="text-micro uppercase tracking-[0.12em] text-fg-muted">
            {row.label}
          </p>
          <div
            class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-fg-muted"
          >
            <span
    ...
    {#if isPresent(rawJson)}
      <div>
        ...
        <div class="mt-1 flex items-start gap-2 text-micro text-fg-muted">
