...
EOF
    emit_env_line HOST_ANX_WORKSPACE_ROOT "$WORKSPACE_ROOT"
    emit_env_line ANX_CORE_PORT "$LISTEN_PORT"
    emit_env_line ANX_WEB_UI_PORT "$WEB_UI_PORT"
    emit_env_line ANX_WEB_UI_ORIGIN "$PUBLIC_ORIGIN"
    emit_env_line ANX_CORE_BASE_URL "$PUBLIC_ORIGIN"
    emit_env_line ANX_WEBAUTHN_RPID "$WEBAUTHN_RPID"
    emit_env_line ANX_WEBAUTHN_ORIGIN "$PUBLIC_ORIGIN"
    emit_env_line ANX_WEBAUTHN_ALLOWED_ORIGINS "$NORMALIZED_ALLOWED_ORIGINS"
    emit_env_line ANX_WEBAUTHN_RP_DISPLAY_NAME "Agent Nexus"
    emit_env_line ANX_CORS_ALLOWED_ORIGINS ""
    emit_env_line ANX_CORE_INSTANCE_ID "$CORE_INSTANCE_ID"
    emit_env_line ANX_WORKSPACE_ID "$CORE_INSTANCE_ID"
    emit_env_line ANX_WORKSPACE_NAME "$INSTANCE_NAME"
    emit_env_line ANX_BOOTSTRAP_TOKEN "$BOOTSTRAP_TOKEN"
    emit_env_line ANX_SHUTDOWN_TIMEOUT "15s"
...
if [[ -f "$INSTANCE_METADATA_FILE" && "$FORCE" -ne 1 ]]; then
  ...
  {
    emit_env_line FORMAT_VERSION "$HOSTED_INSTANCE_FORMAT_VERSION"
    emit_env_line PROVISIONED_AT "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    emit_env_line INSTANCE_NAME "$INSTANCE_NAME"
    emit_env_line INSTANCE_ROOT "$INSTANCE_ROOT"
    emit_env_line WORKSPACE_ROOT "$WORKSPACE_ROOT"
    emit_env_line BACKUPS_DIR "$BACKUPS_DIR"
    emit_env_line LISTEN_HOST "$LISTEN_HOST"
    emit_env_line LISTEN_PORT "$LISTEN_PORT"
    emit_env_line WEB_UI_PORT "$WEB_UI_PORT"
    emit_env_line PUBLIC_ORIGIN "$PUBLIC_ORIGIN"
    emit_env_line WEBAUTHN_RPID "$WEBAUTHN_RPID"
    emit_env_line CORE_INSTANCE_ID "$CORE_INSTANCE_ID"
    emit_env_line WORKSPACE_ID "$CORE_INSTANCE_ID"
    emit_env_line WORKSPACE_NAME "$INSTANCE_NAME"
    emit_env_line BOOTSTRAP_TOKEN_CONFIGURED "$(bootstrap_token_configured_state "$BOOTSTRAP_TOKEN")"
