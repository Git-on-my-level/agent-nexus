...
OAR_CORE_INSTANCE_ID=${INSTANCE_NAME}
...
BOOTSTRAP_TOKEN_CONFIGURED=$([[ "$BOOTSTRAP_TOKEN" == "$HOSTED_BOOTSTRAP_PLACEHOLDER" ]] && printf 'placeholder' || printf 'set')
EOF
...
if [[ "$BOOTSTRAP_TOKEN" == "$HOSTED_BOOTSTRAP_PLACEHOLDER" ]]; then
