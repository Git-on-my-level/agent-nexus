...
[[ -f "$DB_PATH" ]] || die "workspace database not found: $DB_PATH (start anx-core once before backing up)"

ACTIVE_BLOB_BACKEND="$(dotenv_get "$ENV_FILE" ANX_BLOB_BACKEND || true)"
ACTIVE_BLOB_ROOT="$(dotenv_get "$ENV_FILE" ANX_BLOB_ROOT || true)"
ACTIVE_BLOB_S3_BUCKET="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_BUCKET || true)"
ACTIVE_BLOB_S3_PREFIX="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_PREFIX || true)"
ACTIVE_BLOB_S3_REGION="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_REGION || true)"
ACTIVE_BLOB_S3_ENDPOINT="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_ENDPOINT || true)"
ACTIVE_BLOB_S3_ACCESS_KEY_ID="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_ACCESS_KEY_ID || true)"
ACTIVE_BLOB_S3_SECRET_ACCESS_KEY="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_SECRET_ACCESS_KEY || true)"
ACTIVE_BLOB_S3_SESSION_TOKEN="$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_SESSION_TOKEN || true)"
ACTIVE_BLOB_S3_FORCE_PATH_STYLE="$(normalize_bool_value "$(dotenv_get "$ENV_FILE" ANX_BLOB_S3_FORCE_PATH_STYLE || true)")"
...
CORE_INSTANCE_ID="$(dotenv_get "$ENV_FILE" ANX_CORE_INSTANCE_ID || true)"
BOOTSTRAP_TOKEN="$(dotenv_get "$ENV_FILE" ANX_BOOTSTRAP_TOKEN || true)"
WORKSPACE_MAX_BLOB_BYTES="$(dotenv_get "$ENV_FILE" ANX_WORKSPACE_MAX_BLOB_BYTES || true)"
WORKSPACE_MAX_ARTIFACTS="$(dotenv_get "$ENV_FILE" ANX_WORKSPACE_MAX_ARTIFACTS || true)"
WORKSPACE_MAX_DOCUMENTS="$(dotenv_get "$ENV_FILE" ANX_WORKSPACE_MAX_DOCUMENTS || true)"
WORKSPACE_MAX_REVISIONS="$(dotenv_get "$ENV_FILE" ANX_WORKSPACE_MAX_REVISIONS || true)"
WORKSPACE_MAX_UPLOAD_BYTES="$(dotenv_get "$ENV_FILE" ANX_WORKSPACE_MAX_UPLOAD_BYTES || true)"
...
{
  emit_env_line FORMAT_VERSION "$HOSTED_BACKUP_FORMAT_VERSION"
  emit_env_line CREATED_AT "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  emit_env_line INSTANCE_NAME "$INSTANCE_NAME"
  emit_env_line INSTANCE_FORMAT_VERSION "$INSTANCE_FORMAT_VERSION"
  emit_env_line SOURCE_INSTANCE_ROOT "$INSTANCE_ROOT"
  emit_env_line SOURCE_WORKSPACE_ROOT "$WORKSPACE_ROOT"
  emit_env_line PUBLIC_ORIGIN "$PUBLIC_ORIGIN"
  emit_env_line CORE_INSTANCE_ID "$CORE_INSTANCE_ID"
  emit_workspace_quota_metadata_lines "$WORKSPACE_MAX_BLOB_BYTES" "$WORKSPACE_MAX_ARTIFACTS" "$WORKSPACE_MAX_DOCUMENTS" "$WORKSPACE_MAX_REVISIONS" "$WORKSPACE_MAX_UPLOAD_BYTES"
  emit_env_line SQLITE_BACKUP_PATH "workspace/state.sqlite"
  emit_env_line SQLITE_BACKUP_STRATEGY "sqlite3_dot_backup"
  emit_env_line SQLITE_BACKUP_SHA256 "$SQLITE_BACKUP_SHA256"
  emit_env_line SQLITE_SCHEMA_VERSION "$SQLITE_SCHEMA_VERSION"
  emit_env_line SQLITE_USER_VERSION "$SQLITE_USER_VERSION"
  ...
  emit_env_line BLOB_DIR_PATH "$BACKUP_BLOB_RELATIVE_PATH"
  emit_env_line BLOB_BUNDLE_PATH "$BACKUP_BLOB_RELATIVE_PATH"
  emit_env_line CONFIG_INCLUDED "$CONFIG_INCLUDED"
  emit_env_line CONFIG_ENV_PATH "$CONFIG_ENV_PATH"
  emit_env_line METADATA_DIR_PATH "metadata"
  emit_env_line ARTIFACT_COUNT "$ARTIFACT_COUNT"
  emit_env_line AGENT_COUNT "$AGENT_COUNT"
  emit_env_line INVITE_COUNT "$INVITE_COUNT"
  emit_env_line DOCUMENT_COUNT "$DOCUMENT_COUNT"
  emit_env_line DOCUMENT_REVISION_COUNT "$DOCUMENT_REVISION_COUNT"
  emit_env_line BLOB_HASH_COUNT "$BLOB_HASH_COUNT"
  emit_env_line BLOB_FILE_COUNT "$BLOB_FILE_COUNT"
  emit_env_line BLOB_TOTAL_BYTES "$BLOB_TOTAL_BYTES"
  emit_env_line BOOTSTRAP_STATE "$BOOTSTRAP_STATE"
  emit_env_line VERIFY_ARTIFACT_ID "$VERIFY_ARTIFACT_ID"
  emit_env_line VERIFY_DOCUMENT_ID "$VERIFY_DOCUMENT_ID"
  emit_env_line VERIFY_DOCUMENT_REVISION_ID "$VERIFY_DOCUMENT_REVISION_ID"
  emit_env_line CHECKSUM_FILE "SHA256SUMS"
} >"$MANIFEST_FILE"
