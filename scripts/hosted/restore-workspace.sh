...
if [[ "$SOURCE_CONFIG_INCLUDED" == "true" && -f "$SOURCE_ENV_FILE" ]]; then
  SOURCE_BOOTSTRAP_TOKEN="$(dotenv_get "$SOURCE_ENV_FILE" ANX_BOOTSTRAP_TOKEN || true)"
fi
...
SOURCE_WORKSPACE_MAX_REVISIONS="$(manifest_get "$SOURCE_MANIFEST" WORKSPACE_MAX_REVISIONS || true)"
...
if [[ "$SOURCE_CONFIG_INCLUDED" == "true" && -f "$SOURCE_ENV_FILE" ]]; then
  SOURCE_BLOB_S3_ACCESS_KEY_ID="$(dotenv_get "$SOURCE_ENV_FILE" ANX_BLOB_S3_ACCESS_KEY_ID || true)"
  SOURCE_BLOB_S3_SECRET_ACCESS_KEY="$(dotenv_get "$SOURCE_ENV_FILE" ANX_BLOB_S3_SECRET_ACCESS_KEY || true)"
  SOURCE_BLOB_S3_SESSION_TOKEN="$(dotenv_get "$SOURCE_ENV_FILE" ANX_BLOB_S3_SESSION_TOKEN || true)"
fi
...
if [[ -n "$SOURCE_WORKSPACE_MAX_REVISIONS" ]]; then
  provision_args+=(--max-revisions "$SOURCE_WORKSPACE_MAX_REVISIONS")
fi
...
{
  emit_env_line RESTORED_AT "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  emit_env_line SOURCE_BACKUP_DIR "$BACKUP_DIR"
  emit_env_line SOURCE_INSTANCE_NAME "$SOURCE_INSTANCE_NAME"
  emit_env_line SOURCE_INSTANCE_ROOT "$SOURCE_INSTANCE_ROOT"
  emit_env_line SOURCE_WORKSPACE_ROOT "$SOURCE_WORKSPACE_ROOT"
  emit_env_line SOURCE_PUBLIC_ORIGIN "$SOURCE_PUBLIC_ORIGIN"
  emit_env_line TARGET_INSTANCE_NAME "$INSTANCE_NAME"
  emit_env_line TARGET_INSTANCE_ROOT "$TARGET_INSTANCE_ROOT"
  emit_env_line TARGET_WORKSPACE_ROOT "$TARGET_WORKSPACE_ROOT"
  emit_env_line TARGET_PUBLIC_ORIGIN "$PUBLIC_ORIGIN"
  emit_env_line TARGET_LISTEN_HOST "$LISTEN_HOST"
  emit_env_line TARGET_LISTEN_PORT "$LISTEN_PORT"
  emit_env_line TARGET_WEB_UI_PORT "$WEB_UI_PORT"
  emit_env_line TARGET_CORE_INSTANCE_ID "$CORE_INSTANCE_ID"
  emit_env_line TARGET_BLOB_BACKEND "$SOURCE_BLOB_BACKEND"
  emit_env_line TARGET_BLOB_STORAGE_MODE "$SOURCE_BLOB_STORAGE_MODE"
  emit_env_line TARGET_BLOB_ROOT "$TARGET_BLOB_ROOT"
  emit_env_line TARGET_BLOB_EFFECTIVE_LOCATION "$TARGET_EFFECTIVE_BLOB_LOCATION"
  emit_env_line TARGET_BLOB_BUNDLE_PATH "$SOURCE_BLOB_BUNDLE_PATH"
  emit_env_line TARGET_BLOB_RESTORE_ACTION "$TARGET_BLOB_RESTORE_ACTION"
  emit_env_line TARGET_BLOB_S3_BUCKET "$SOURCE_BLOB_S3_BUCKET"
  emit_env_line TARGET_BLOB_S3_PREFIX "$SOURCE_BLOB_S3_PREFIX"
  emit_env_line TARGET_BLOB_S3_REGION "$SOURCE_BLOB_S3_REGION"
  emit_env_line TARGET_BLOB_S3_ENDPOINT "$SOURCE_BLOB_S3_ENDPOINT"
  emit_env_line TARGET_BLOB_S3_FORCE_PATH_STYLE "$SOURCE_BLOB_S3_FORCE_PATH_STYLE"
  emit_env_line TARGET_BLOB_S3_INLINE_CREDENTIALS_PRESENT "$(blob_s3_inline_credentials_present "$SOURCE_BLOB_S3_ACCESS_KEY_ID" "$SOURCE_BLOB_S3_SECRET_ACCESS_KEY" "$SOURCE_BLOB_S3_SESSION_TOKEN")"
  emit_env_line BOOTSTRAP_TOKEN_MODE "$BOOTSTRAP_TOKEN_MODE"
  emit_env_line EXPECTED_ACTIVE_BOOTSTRAP_STATE "$EXPECTED_ACTIVE_BOOTSTRAP_STATE"
  emit_env_line FORCE_MODE "$([[ "$FORCE" -eq 1 ]] && printf 'true' || printf 'false')"
} >"${TARGET_METADATA_DIR}/restore-receipt.env"
